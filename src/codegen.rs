    ...
    fn compile_top_level(&mut self, func: IRTopLevel) -> Vec<String> {
        ...
        for op in func.ops {
            match &op {
                ...
                IROp::CopyWithOffset((source, source_offset), (destination, destination_offset)) => {
                    self.builder.copy_with_offset((source, *source_offset), (destination, *destination_offset));
                }
